...
**Date:** March 7, 2026
**Version:** 1.1 (updated with second review pass findings)
...
- **This week:** Quick wins (webhook auth, status normalization, sanitization) — ✅ DONE
...
- **During Phase 1:** RUNBOOK.md, database migration plan, multi-tenant architecture design — ✅ RUNBOOK DONE
- **Phase 2 trigger:** When client count hits 8 or Google Sheets API errors > 5/day

---

## Addendum: Second Review Pass — Regulatory & Compliance Findings

The second review pass identified additional regulatory risks not captured in the initial assessment:

### EU AI Act Compliance (CRITICAL — August 2, 2026 Deadline)
- Mandatory AI content labeling takes effect August 2, 2026
- All blog posts, LinkedIn posts, and tweets generated by the content pipeline would be non-compliant
- Penalties: EUR 15 million or 3% of global turnover
- No workflows implement C2PA metadata or human-readable disclosure injection
- **Action:** Build dual-layer AI disclosure (human-readable + C2PA machine-readable) into content pipeline by June 2026
- **Strategic opportunity:** Be the first AI content platform to implement compliant disclosure — market as a feature

### CAN-SPAM Exposure (HIGH)
- `email-sequences.json` workflow sends emails without CAN-SPAM compliance checks
- Per-email penalties of $51,744 with no cap
- Both SeedLink and the client are legally liable
- **Action:** Add CAN-SPAM validation node to `email-sequences.json` (physical address, unsubscribe link, honest subject line)

### LinkedIn ToS Risk (HIGH — Workstream A)
- LinkedIn User Agreement explicitly prohibits automated methods
- 23% ban rate for automation users reported in 2026
- At 10+ clients, probability of at least one account restriction is "near-certainty"
- No professional liability (E&O) insurance in place
- **Action:** Obtain E&O insurance before first client. Build email-first outreach as fallback channel.

### AI Content Copyrightability (MEDIUM)
- US Copyright Office: purely AI-generated content is not copyrightable (*Thaler v. Perlmutter*)
- Clients paying $1,500-$3,000 may assume copyrightable ownership of generated content
- **Action:** Add clear IP disclosure to client agreements — AI-generated content may not receive copyright protection

### CCPA Data Broker Classification (MEDIUM)
- If SeedLink is classified as a data broker under California's Delete Act, registration is mandatory
- $200/day penalties for non-compliance
- **Action:** Conduct assessment with privacy counsel before accepting California-based clients

### Quick Wins Completed This Session
| Item | Status |
|------|--------|
| Webhook authentication on all endpoints | ✅ Implemented |
| Anti-prompt-injection safety in all 14 prompts | ✅ Implemented |
| Lite Support scope defined (≤5 hrs/month) | ✅ Added to sop.md |
| Content failure procedure (after 2 revisions) | ✅ Added to sop.md |
| Client Quick Start 1-pager | ✅ CLIENT_QUICK_START.md |
| Operational Runbook (17 scenarios) | ✅ RUNBOOK.md |
| CHANGELOG for tracking changes | ✅ CHANGELOG.md |
| Stripe Connect payout fees in revenue examples | ✅ REVENUE_STRUCTURE.md |
| REQUIREMENTS.md v1 historical note | ✅ Added |
| SKILL.md expanded to 16 lenses | ✅ Done |
| Google Form for client onboarding | ✅ onboarding-form/ |
| Partnership call talking points | ✅ PARTNERSHIP_CALL_TALKING_POINTS.md |
