...
| **Settings** | Client configuration | Brand name, pillars, blog platform, LinkedIn accounts |
| **Voice Profile** | Brand voice configuration | Generated by Voice Builder or Voice Extractor during onboarding. Protected tab — do not manually edit after calibration. |
...
| `voice-builder.md` | Onboarding (Voice Builder path) | Questionnaire responses → structured Voice Profile (for clients with no existing content) |
| `voice-extractor.md` | Onboarding (Content Analysis path) | Existing content samples → structured Voice Profile (for clients with published content) |
