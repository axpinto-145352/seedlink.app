...
3. SeedLink pays VV 75% "within 7 business days"
...
- When a client pays, Stripe automatically splits the payment at checkout:
  - 75% → VV's connected account (1-business-day delayed payout)
  - 25% → SeedLink's platform account (deposited same day)
...
Net to split:        $2,130.10
  → SeedLink (25%):  $532.53   (deposited same day — logged as revenue)
  → VV (75%):        $1,597.58 (auto-deposited 1 business day later)
```

> **Delayed split rationale (per Mar 9 call):** Payment lands in SeedLink's account first so it is logged as revenue for investor reporting purposes. Stripe Connect is configured with a 1-business-day delay before VV's portion is released.
...
3. **SeedLink absorbs** — comes off SeedLink's 25% since they chose the payment platform (simplest)
...
5. Set up payment splits in checkout sessions:
   ...
   // In Stripe checkout session creation:
   {
     payment_intent_data: {
       ...
       application_fee_amount: Math.round(amount * 0.25), // SeedLink keeps 25%
     }
   }
   ```
...
- SeedLink's 25% released simultaneously
...
- VV invoices the client directly for 75% of the build
- SeedLink invoices the client for 25% referral fee
...
> - SeedLink receives funds same day; VV payout delayed by 1 business day (for SeedLink revenue logging)
...
> - Stripe processing fees absorbed proportionally (75% by VV, 25% by SeedLink)
